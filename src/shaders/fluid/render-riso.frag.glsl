...
// Coverage of one plate: hard threshold on dye density sampled at a
// misregistered offset, with the sample point wobbled by simplex
// noise so the print edge frays like absorbed ink. All plates share
// one noise pair (sign-flipped per plate) -- 8 snoise calls per
// fragment bought nothing visible over 2.
float plate(vec2 uv, vec2 offsetTexels, float lo, float hi, vec2 bleed) {
  vec2 misreg = uTexelSize * offsetTexels;
  vec3 dye = clamp(texture(uDye, uv + misreg + bleed).rgb, vec3(0.0), vec3(1.0));
  return smoothstep(lo, hi, length(dye));
}
...
void main() {
  ...
  float mMint   = plate(vUv, vec2( 6.0,  3.0), 0.05, 0.12,  bleedN);
  float mAmber  = plate(vUv, vec2(-5.0,  6.0), 0.20, 0.28, -bleedN);
  float mRose   = plate(vUv, vec2( 4.0, -6.0), 0.38, 0.47, vec2( bleedN.y, -bleedN.x));
  float mViolet = plate(vUv, vec2(-6.0, -4.0), 0.58, 0.68, vec2(-bleedN.y,  bleedN.x));
