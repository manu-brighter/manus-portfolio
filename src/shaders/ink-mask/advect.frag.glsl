...
void main() {
  ...
  // Use a low-frequency curl field so ink drifts in soft eddies, not
  // chaos. Time-varying so still a visible motion after the splat
  // burst settles.
  vec2 vel = curl(vUv * 2.5 + vec2(0.0, uTime * 0.05)) * 0.6;
