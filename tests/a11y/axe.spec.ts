...
test.describe("@a11y axe — home", () => {
  test("default motion preference — no WCAG AA violations", async ({ page }) => {
    ...
    expect(results.violations, "axe violations on / (default motion)").toEqual([]);
  });
  ...
  test("reduced-motion — no WCAG AA violations", async ({ browser }) => {
    ...
    try {
      ...
      const results = await new AxeBuilder({ page })
        .withTags(["wcag2a", "wcag2aa", "wcag21a", "wcag21aa", "best-practice"])
        .analyze();
      expect(results.violations, "axe violations on / (reduced-motion)").toEqual([]);
    } finally {
      await context.close();
    }
  });
